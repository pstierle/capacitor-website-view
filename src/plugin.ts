...
const WebviewOverlayPlugin = registerPlugin<IWebviewOverlayPlugin>(
...
class WebviewOverlayClass {
  ...
  open(options: WebviewOverlayOpenOptions): Promise<void> {
    ...
    this.updateSnapshotEvent = WebviewOverlayPlugin.addListener(
      'updateSnapshot',
      () => {
        setTimeout(() => {
  ...
  async toggleSnapshot(snapshotVisible: boolean): Promise<void> {
    return new Promise<void>(async resolve => {
      ...
      if (snapshotVisible) {
        if (snapshot) {
          const buffer = await (
            await fetch('data:image/jpeg;base64,' + snapshot)
          ).arrayBuffer();
  ...
  onPageLoaded(listenerFunc: () => void) {
    this.pageLoadedEvent = WebviewOverlayPlugin.addListener(
      'pageLoaded',
      listenerFunc,
    );
  }
  ...
  onProgress(listenerFunc: (progress: { value: number }) => void) {
    this.progressEvent = WebviewOverlayPlugin.addListener(
      'progress',
      listenerFunc,
    );
  }
  ...
  handleNavigation(
    ...
    this.navigationHandlerEvent = WebviewOverlayPlugin.addListener(
      'navigationHandler',
      (event: any) => {
